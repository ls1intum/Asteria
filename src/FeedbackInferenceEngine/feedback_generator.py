...
from FeedbackInferenceEngine.feedback_text import FeedbackText
from KnowledgeComparisonEngine.UMLElement import Class
...
class FeedbackGenerator:
    ...
    def get_feedback(self, similarity_triples: [(Class, Class, float)]) -> ([str], [str]):
        """Generate feedback for a submission
            ...
            :param similarity_triples: similarity triples resulting from the comparison of the student's submission with the sample solution
            :return: List of positive feedback, list of negative feedback
        """
        flatten = itertools.chain.from_iterable
        correct_identified_classes = [class1.name for (class1, class2, _) in similarity_triples if
                                      class1 is not None and class2 is not None]
        correct_classes_feedback = self.feedback_text.get_correctly_identified__classes_feedback(
            correct_identified_classes)
    ...
    def get_feedback_for_similarity_pair(self, similarity_triple: (Class, Class, float)) -> ([str], [str]):
        """Generate feedback for a single element of the submission

            :param similarity_triple: similarity triple resulting from the comparison of the student's submission with the sample solution
            :return: List of positive feedback, list of negative feedback
        """
    ...
    def get_feedback_for_associations(self, submission_element: Class, solution_element: Class) -> ([str], [str]):
        """Generate feedback regarding the associations of a specific class
            :param submission_element: submission element that consists of an association
            :param solution_element:  solution element contained in the same similarity pair as the submission element
            :return: List of positive feedback, list of negative feedback
        """
        associations_relations = ["implements", "implements", "is composed of", "uses", "calls"]
        associations_names = ["inheritance", "realization", "aggregation(i.e. composition)",
                              "unidirectional association", "dependency"]
        ...
        if len(similar_associations_names) > 0:
            positive_feedback.append(
                self.feedback_text.get_associations_partially_correct_feedback(submission_element.name,
        ...
        if len(missing_associations_names) > 0:
            negative_feedback.append(
                self.feedback_text.get_missing_associations_feedback(submission_element.name,
                                                                     missing_associations_names))
    ...
    def get_feedback_for_method(self, submission_element: Class, solution_element: Class) -> ([str], [str]):
        """Generate feedback regarding the methods of a class
            :param submission_element: submission element
            :param solution_element:  solution element contained in the same similarity pair as the submission element
            :return: List of positive feedback , list of negative feedback
        """
    ...
    def get_feedback_for_attribute(self, submission_element: Class, solution_element: Class) -> ([str], [str]):
        """Generate feedback regarding the attributes of a class
            :param submission_element: submission element
            :param solution_element:  solution element contained in the same similarity pair as the submission element
            :return: List of positive feedback, list of negative feedback
        """
