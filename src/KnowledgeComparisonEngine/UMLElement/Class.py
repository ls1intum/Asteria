...
class Class(UMLElement):
    ...
    def __get_intersection(self, lst1, lst2):
        # calculate the intersection between two lists
        temp = set(lst2)
        lst3 = [value for value in lst1 if value in temp]
        return lst3

    def __calculate_methods_similarity(self, reference_class: 'Class', targets1_list: [str], targets2_list: [str]) \
            -> float:
        """
        calculate the similarity between two classes based on the methods that they both contain
        """
    ...
    def __calculate_attributes_similarity(self, reference_class: 'Class'):
        """
         calculate the similarity between two classes based on their attributes
        """
    ...
    def calculate_similarity(self, reference_class: 'Class', targets1_list,
        ...
        association_indices = [self.similarity_options.RELATIONS.index(association) + 1 for association in
                               self.similarity_options.UML_ASSOCIATIONS]
